...
class AllFlagsQuiz: UIViewController, UITextFieldDelegate{
    ...
    let allQuestions = FlagEmoji()
    var pickedAnswer = ""
    var lvlNumber = 0
    var score = 0
    let scoreLabel = FlagScoreLabel().scoreLabel()
    let lvlEmoji = FlagLevelEmoji().lvlEmoji()
    ...
    let previousAnswerLabel = FlagPreviousAnswerLabel().previousAnswerLabel()
    var previousEmoji = ""
    ...
    func nextQuestion(){
        ...
        if lvlNumber < numberOfQuestions{
            ...
            if score > defaults.integer(forKey: "allFlagsHighScore"){
                highscore = score
                defaults.set(highscore, forKey: "allFlagsHighScore")
            ...
            let restartAction = UIAlertAction(title: "Go Back", style: .default, handler: { (UIAlertAction) in
                self.show(FlagQuizzesOptions(), sender: self)
            })
