...
class WorldCup2018Quiz: UIViewController, UITextFieldDelegate{
    ...
    let allQuestions = WorldCup2018Emoji()
    var pickedAnswer = ""
    var lvlNumber = 0
    var score = 0
    let scoreLabel = WorldCup2018ScoreLabel().scoreLabel()
    let lvlEmoji = WorldCup2018LevelEmoji().lvlEmoji()
    ...
    let previousAnswerLabel = WorldCup2018PreviousAnswerLabel().previousAnswerLabel()
    var previousEmoji = ""
    ...
    func nextQuestion(){
        ...
        if lvlNumber < numberOfQuestions{
            if lvlNumber == 0{
                ...
                randomNumber = Int(arc4random_uniform(UInt32(allQuestions.list.count)))
                lvlEmoji.text = allQuestions.list[randomNumber].emoji
                previousAnswerLabel.text = "Previous Flag: \(previousEmoji)\nName: \(previousEmojiName)"
            ...
            previousAnswerLabel.text = "Previous Flag: \(previousEmoji) Name: \(previousEmojiName)"
            previousAnswerText = previousAnswerLabel.text!
            gameTimer.invalidate()
            
            if score > defaults.integer(forKey: "russia2018HighScore"){
                highscore = score
                defaults.set(highscore, forKey: "russia2018HighScore")
            ...
            let restartAction = UIAlertAction(title: "Go Back", style: .default, handler: { (UIAlertAction) in
                self.show(WorldCupQuizzesOptions(), sender: self)
            })
    ...
    @objc func gameTimerStart() {
        ...
        if lvlNumber == numberOfQuestions{
            ...
            nextQuestion()
            gameTimeLeft = 10
            gameTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(AllFlagsQuiz.gameTimerStart), userInfo: nil, repeats: true)
        }
    }
    ...
    private let lvlTextField: UITextField = {
        ...
        text.placeholder = "Enter flag name"
    ...
    @objc func handlePrev(){
        gameTimer.invalidate()
        let previosScreen = WorldCupQuizzesOptions()
