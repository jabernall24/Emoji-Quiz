...
class AnimalQuiz: UIViewController, UITextFieldDelegate{
    ...
    var allQuestions = AnimalEmoji()
    var pickedAnswer = ""
    var lvlNumber = 0
    var score = 0
    let scoreLabel = AnimalScoreLavel().scoreLabel()
    let lvlEmoji = AnimalLevelEmoji().lvlEmoji()
    ...
    let previousAnswerLabel = AnimalPreviousAnswerLabel().previousAnswerLabel()
    var previousEmoji = ""
    ...
    func nextQuestion(){
        ...
        if lvlNumber < numberOfQuestions{
            if lvlNumber == 0{
                ...
                randomNumber = Int(arc4random_uniform(UInt32(allQuestions.list.count)))
                lvlEmoji.text = allQuestions.list[randomNumber].emoji
                previousAnswerLabel.text = "Previous emoji: \(previousEmoji)\nName: \(previousEmojiName)"
            ...
            if score > defaults.integer(forKey: "animalHighScore"){
                highscore = score
                defaults.set(highscore, forKey: "animalHighScore")
                defaults.synchronize()
            }
            
            previousAnswerLabel.text = "Previous emoji: \(previousEmoji) Name: \(previousEmojiName)"
            previousAnswerText = previousAnswerLabel.text!
    ...
    @objc func gameTimerStart() {
        ...
        if lvlNumber == numberOfQuestions{
            ...
            nextQuestion()
            gameTimeLeft = 10
            gameTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(AnimalQuiz.gameTimerStart), userInfo: nil, repeats: true)
        }
    }
    ...
    private let lvlTextField: UITextField = {
        ...
        text.placeholder = "Enter name of the animal"
        text.translatesAutoresizingMaskIntoConstraints = false
        return text
    }()
    
    func checkAnswer() {
        let lowerCase = lvlTextField.text?.lowercased()
        let emojiAnswer = allQuestions.list[randomNumber]
        if lowerCase == emojiAnswer.emojiName || lowerCase == emojiAnswer.secondEmojiName || lowerCase == emojiAnswer.thirdEmojiName{
    ...
    private func easyLvlSetUpLayout() {
